...
export const StrategyPanel = React.createClass({
  ...
  onUpdate(){
    Transact.createOrUpdateStrategy((res)=>{
      console.log(res)
    }, this.state.chosenStrategy, this.state.code)
  },
  ...
  notifyResult(result){
    this.setState({
      ...
      this.resultFold.toggleFold(false)
    })
  },
  ...
  render(){
    return (<div>
      <div style = {strategyContainerStyle}>
        <Foldable label={'Strategies'} ref={f=>{this.resultFold=f}} >
          <StrategyResult result={this.state.result}/>
        </Foldable>
